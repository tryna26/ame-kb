...
one or more integer versions. `apply_graph_context` sets the process-wide
GRAPH_NO / GRAPH_VERSION env vars and clears the settings cache so every
downstream `get_settings()` sees the selected graph — this is how the CLI
callback injects `--graph-no/--graph-version` without touching the ~46 call
sites.
...
def apply_graph_context(
    ...
    """Set GRAPH_NO / GRAPH_VERSION for this process and refresh settings.

    - graph_no None/empty -> leave the default in place (no version lookup).
    - graph_version None  -> resolve to the graph's latest version (fault
      tolerant: any DB/lookup error falls back to the default version).
    """
    if graph_no:
        os.environ["GRAPH_NO"] = graph_no
    active_no = graph_no or os.environ.get("GRAPH_NO", DEFAULT_GRAPH_NO)

    if graph_version is not None:
        os.environ["GRAPH_VERSION"] = str(graph_version)
    elif active_no != DEFAULT_GRAPH_NO:
        ...
        latest = _latest_version_safe(active_no)
        if latest is not None:
            os.environ["GRAPH_VERSION"] = str(latest)

    get_settings.cache_clear()


def _latest_version_safe(graph_no: str) -> Optional[int]:
    """MAX(graph_version) for a graph, or None if unavailable.
...
def latest_version(graph_no: str) -> Optional[int]:
    """MAX(graph_version) for a graph (propagates real errors). None if none."""
    with session_scope() as session:
        return session.execute(
            select(func.max(Graph.graph_version)).where(
                Graph.graph_no == graph_no
