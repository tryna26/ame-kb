...
from .config import get_default_settings, graph_context
...
def _resolve_graph(
    ...
    """Resolve (graph_no, version) the same way the CLI callback does.

    graph_no None/empty -> environment default. graph_version None -> the
    graph's latest ACTIVE version (default graph keeps its configured version and
    never triggers a kg_graph lookup, mirroring graphs.apply_graph_context).
    """
...
def search(
    ...
    *,
    ...
    with _graph_scope(graph_no, graph_version):
        return _recall(query, window=window, trace=trace)
...
def resolve_all(
    *,
    ...
    with _graph_scope(graph_no, graph_version):
        return _resolve_all(type_filter=type_filter, dry_run=dry_run, limit=limit)
